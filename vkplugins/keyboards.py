...
class keyboards:
	...
	start = (Keyboard(one_time=True, inline=False)
		.add(Text('Меню', payload={'menu': 0}), color=KeyboardButtonColor.POSITIVE)
	).get_json()
	...
	menu_reg = (Keyboard(one_time=True, inline=False)
		.add(Text('Регистрация', payload={'reg': 0}), color=KeyboardButtonColor.POSITIVE)
	).get_json()
	back = (Keyboard(one_time=True, inline=False)
		.add(Text('Вернуться назад', payload={'menu': 0}), color=KeyboardButtonColor.POSITIVE)
	).get_json()
	pay = (Keyboard()
